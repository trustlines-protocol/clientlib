    ...
    describe('#messageStream()', () => {
      ...
      it('should receive payment requests', (done) => {
        ...
        setTimeout(() => {
          ...
          expect(messages[0]).to.have.property('type', 'PaymentRequest')
          expect(messages[0].amount).to.have.keys('raw', 'value', 'decimals')
          expect(messages[0]).to.have.nested.property('amount.value', 250)
          expect(messages[0].timestamp).to.be.a('number')
          expect(messages[0]).to.have.property('from', user2.address)
          expect(messages[0]).to.have.property('to', user1.address)
